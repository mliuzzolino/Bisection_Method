...
class Bisection(object):
	...
	def run(self):
		...
		while self.num_real_pos_solutions:
			...
			self.find_roots(self.num_real_pos_solutions)
			print(self.roots)
		...
		while self.num_real_neg_solutions:
			...
			# Check if odd or even. Odd -> at least 1 solution exists
			self.find_roots(self.num_real_neg_solutions)
			print(self.roots)
	...
	def update_num_real_pos_solutions(self, possible_solutions, adjust_factor):

		new_num_real_pos_solutions = []
		for num in possible_solutions:
			new_num = num - adjust_factor
			if new_num >= 0:
				new_num_real_pos_solutions.append(new_num)

		self.num_real_pos_solutions = new_num_real_pos_solutions[:]

	

	def update_domains(self):
		new_positive_domains = []
		for domain in self.positive_domains:
			A_i = domain[0]
			B_i = domain[1]

			if self.x_ns > A_i and self.x_ns < B_i:
				new_positive_domains.append((A_i, self.x_ns))
				new_positive_domains.append((self.x_ns, B_i))

			else:
				new_positive_domains.append((A_i, B_i))

		self.positive_domains = new_positive_domains[:]



	def find_roots(self, possible_solutions):
		...
		# Odd case
			...
			for domain in self.positive_domains:
				...
				self.update_num_real_pos_solutions(possible_solutions, 1)
		...
		# Even case!
			...
			for domain in self.positive_domains:
				...
				# root doesn't exist
					...
					# Update possible_solutions
					self.update_num_real_pos_solutions(possible_solutions, 2)


		# Recursion!
		self.find_roots(possible_solutions)
